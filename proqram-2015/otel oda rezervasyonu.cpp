...
class otel{
	...
	int toplam,sayac1,sayac2,gun[45];
	enum status{
		bos,dolu
		}odalar[45];
	public:
...
void otel::oda_sifaris(){
	...
	switch(sayi){
		...
		system("cls");
			...
			if(k==16) cout<<"Teessufki butun 2 otaqlilar doludur.\n";
		}cout<<"\nBas MENU ya getmek ucun her hansi bir duymeni sixin...";
			p=getch();}
		break;
		case 3:{int p;
...
void otel::faylayaz(){
	int i;
	ofstream fayl("d:\\faylim.text");
	for(i=0;i<46;i++){
		if(odalar[i]==dolu)
		fayl<<"Otaq "<<i<<": "<<*musteriadi[i]<<"\t"<<*soyadi[i]<<"\tkiraye muddeti: "<<gun[i]<<" gun\n";
	}
	cout<<"\nMelumat Fayla Yazildi...";
	cout<<"\n\nBas MENU ya getmek ucun her hansi bir duymeni sixin...";
			i=getch();
}
